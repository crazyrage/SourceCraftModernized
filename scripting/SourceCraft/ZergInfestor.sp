...
static const char[] g_PlagueSound[] = "sc/zdeblo01.wav";
...
void SetupSwarmInfestation(client, level)
{
    if (m_GasAvailable)
    {
    ...
    if (m_GasAvailable)
    {
        ...
        else if (CanInvokeUpgrade(client, raceID, swarmID, false))
        {
            ...
            if (GasAttack(client))
            {
                ...
                {
                    int float cooldown = GetUpgradeCooldown(raceID, swarmID);
                    if (cooldown > 0.0)
                    {
...
void Corruption(client, level)
{
    ...
    else if (CanInvokeUpgrade(client, raceID, corruptionID, false))
    {
        ...
        int float range = g_CorruptionRange[level];
...
public Action CorruptionTimer(Handle timer,Handle pack)
{
    if (pack != null)
    {
        ...
        int float endTime=ReadPackFloat(pack);
