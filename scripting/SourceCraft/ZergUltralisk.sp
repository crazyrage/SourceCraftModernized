...
static const char[] deathWav[] = "sc/zuldth00.wav";
...
static const char[] g_ChargeSound[] = "sc/zulror00.wav";
static const char[] g_ChargeAttackSound[][] = { "sc/zulatt00.wav" ,
                                             "sc/zulatt01.wav" ,
                                             "sc/zulatt02.wav" };
...
void SetupSpeedAndGravity(client, armor_level, speed_level, bool apply=false)
{
    ...
    if (speed >= 0.0 && speed != 1.0)
    {
        ...
        #if defined _TRACE
            int float calc = speed;
        #endif

        // Make slowest base speed 200 (Heavy == 230)
        int float limit = (TF2_IsPlayerSlowed(client) ? 80.0 : 200.0)
...
void Cleave(client, level)
{
    ...
    else if (CanInvokeUpgrade(client, raceID, cleaveID))
    {
        ...
        int float range = 50.0 + float(level)*50.0;
        ...
        int count = 0;
        int float force = float(level) * -75.0;
        int float vertForce = force * -2.0;
        if (vertForce < 300.0)
            vertForce = 300.0;
