...
static const char[] buildWav[]         = "sc/tverdy00.wav";  // Spawn sound
...
void EMPShockWave(client, emp_level)
{
    if (GetRestriction(client,Restriction_NoUltimates) ||
        GetRestriction(client,Restriction_Stunned))
    {
...
void SetupUberCharger(client, level)
{
    if (m_UberChargerAvailable)
    {
        if (level > 0)
