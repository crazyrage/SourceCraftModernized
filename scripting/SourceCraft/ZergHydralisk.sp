...
static const char[] g_MissileAttackSound[]  = "sc/spooghit.wav";
...
public Action OnRaceDeselected(client,oldrace,newrace)
{
    if (oldrace == raceID)
    {
        ...
        if (m_PoisonActive[client])
            EndPoison(null, GetClientUserId(client));
                          ...
                          bool headshot, bool backstab, bool melee)
{
    if (victim_race == raceID)
    {
        if (m_PoisonActive[victim_index])
            EndPoison(null, GetClientUserId(victim_index));
    }
    else
    {
        if (g_lurkerRace < 0)
