...
static const char[] bootsWav[]  = "sc/bootospeed.mp3";
static const char[] tomeSound[] = "sc/tomes.wav";
...
Handle g_BootTimers[MAXPLAYERS+1] = { null, ... };
Handle g_NadeTimers[MAXPLAYERS+1] = { null, ... };
Handle g_AmmoPackTimers[MAXPLAYERS+1] = { null, ... };
Handle g_RegenerationTimers[MAXPLAYERS+1] = { null, ... };

Handle g_TrackWeaponsTimers[MAXPLAYERS+1] = { null, ... };
Handle vecPlayerWeapons[MAXPLAYERS+1] = { null, ... };
int myWepsOffset = 0;
...
Handle hUTILRemove    = null;
Handle hWeaponDrop    = null;
Handle hSetModel      = null;
...
public OnSourceCraftReady()
{
    ...
    cfgAllowInvisibility            = view_as<bool>(GetConfigNum("allow_invisibility", cfgAllowInvisibility));
...
public OnPlayerAuthed(client)
{
    ...
    if (GameTypeIsCS() && vecPlayerWeapons[client] == null)
    {
...
public Action OnItemPurchaseEx(int client, int item, bool &use_pcrystals, int &cost, int &cost_vespene, int &cost_xp)
{
    Action returnCode = Plugin_Continue;
...
public Action OnXPGiven(int client, int &amount, bool taken)
{
...
public Action OnInfected(int victim, int infector, int source, bool infected, const int color[4])
{
    if (infected)
    {
...
public Action OnSetTripmine(int client)
{
...
public Action OnSetMine(int client)
{
    if (m_IsMole[client] || GetAttribute(client, Attribute_IsAMole))
    {
        PrepareAndEmitSoundToClient(client,deniedWav);
...
public Action ItemCommand(int client, int args)
{
        ...
        if (GetArraySize(playerList) > 0) // are there any enemies?
        {
            ...
            else
            {
                ...
                if (TeleportPlayer(client,teleLoc, NULL_VECTOR, NULL_VECTOR))
                {
                    ...
                    {
                        for (TFCond c = TFCond_CritHype;c <= TFCond_CritOnKill; c++)
                        {
...
void UsePeriapt(client)
{
    IncreaseHealth(client, 50);
    m_UsedPeriapt[client]=true;
}

void LootCorpse(Handle event,victim_index, index)
{
...
{
    if (g_RegenerationTimers[client] == null)
...
stock KillRegenerationTimer(client)
{
    if (g_RegenerationTimers[client] != null)
    {
        ...
        g_RegenerationTimers[client] = null;  
    }
}

stock CreateNadeTimer(client)
{
    if (g_NadeTimers[client] == null)
...
stock KillBootTimer(client)
{
    if (g_BootTimers[client] != null)
    {
        ...
        g_BootTimers[client] = null;  
    }
}

stock CreateAmmoPackTimer(client)
{
    if (g_AmmoPackTimers[client] == null)
...
stock KillAmmoPackTimer(client)
{
    if (g_AmmoPackTimers[client] != null)
    {
        ...
        g_AmmoPackTimers[client] = null;  
    }
}

stock CreateTrackWeaponsTimer(client)
{
    if (g_TrackWeaponsTimers[client] == null)
...
stock KillTrackWeaponsTimer(client)
{
    if (g_TrackWeaponsTimers[client] != null)
    {
        ...
        g_TrackWeaponsTimers[client] = null;  
    }
}
...
stock SetModel(entity,const char[] model[])
{
    SDKCall(hSetModel,entity,model);
}

stock Handle PlayersOnTeam(int team)
{
...
// Natives
//
public int Native_IsMole(Handle plugin,numParams)
{
    int client = GetNativeCell(1);
    return m_IsMole[client];
}

public int Native_SetMole(Handle plugin,numParams)
{
    int client = GetNativeCell(1);
...
public int Native_InvokeMole(Handle plugin,numParams)
{
    int client = GetNativeCell(1);
    DoMole(null, GetClientUserId(client));
}
