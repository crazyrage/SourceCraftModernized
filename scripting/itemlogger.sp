...
public Action Event_item_found(Handle event, const char[] name, bool dontBroadcast)
{
	...
	{
		char item[128]; char item_esc[512]; char client_name[32]; char client_auth[32]; char client_name_esc[128]; char client_auth_esc[128]; char query[1024];
		//Client Info
		GetClientName(userid, client_name, sizeof(client_name));
		GetClientAuthString(userid, client_auth, sizeof(client_auth));
		//Item Info
		GetEventString(event, "item", item, sizeof(item));
		int method = GetEventInt(event, "method");
		int quality = GetEventInt(event, "quality");
		//Escaping
