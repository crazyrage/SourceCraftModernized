...
public bool MakeBuster(client, toggle)
{
	...
	if (toggle == 2 || (toggle == 1 && !isBuster[client]))
	{
		...
		int wepEnt = SpawnWeapon(client, "tf_weapon_stickbomb", 307, 10, 6, "26 ; 2325 ; 107 ; 2.0 ; 252 ; 0.5 ; 329 ; 0.5 ; 330 ; 7 ; 402 ; 1"); // Yes, SpawnWeapon. VSH. Hi Flamin.
...
stock bool AttachParticle(ent, char particleType[], bool cache=false)
{
...
stock SpawnWeapon(client, char name[], itemIndex, level, qual, char att[])
{
